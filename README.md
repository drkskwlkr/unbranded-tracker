...
- A1 Post
- Лео Експрес
- CVC (**към януари 2022 г. — вече не, поради смяна на системата. Работя по възстановяване на функционалността.**)
- Български пощи
- EMS Bulpost
- Elta (Hellenic Post)
...
## Инсталация
Трябва да свалите 6+1 файла от GitHub:
```
access.inc.php
config.inc.php
index.php
...
track.php
feedback.php
.htaccess.DISABLED (този е по желание; как се ползва е посочено по-долу в "Алтернативен метод на употреба")
```
...
Това решение има някои ограничения спрямо оригиналното поведение на скрипта; прочетете ги внимателно:
...
Ако сте окей с това, свалете файла `.htaccess.DISABLED` и го преименувайте на `.htaccess` (не пропускайте точката отпред). Тествайте тракера. Ако забелязвате проблем или виждате грешка HTTP 500, преименувайте обратно `.htaccess` на нещо друго или го изтрийте, и тракерът ще възстанови първоначалното си поведение.
...
Най-добре, разбира се, е да тествате със собствена товарителница.
